...
# Load teacher model
teacher_model = model_dict['resnet110'](num_classes=10)  # Update num_classes to 10 for CIFAR-10
...
if config['SAVE']:
    os.makedirs(config['SAVE_DIR'], exist_ok=True)  # Ensure the save directory exists
    torch.save(student.state_dict(), os.path.join(config['SAVE_DIR'], config['NAME']))
